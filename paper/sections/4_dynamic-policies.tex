...
\caption{Policy performance/utilization shows the trade-offs of different sized caches
(\(x\) axis).  ``None" is ParSplice unmodified, ``Fixed Sized Cache" evicts
keys using LRU, and ``Multi-Policy Cache" switches to fixed sized cache after
absorbing the workload's initial burstiness.  \label{fig:methodology-tradeoff}}
\end{figure}
...
Policy" saves the most memory without sacrificing performance.
