...
Figure~\ref{src:lru-dyn}, but the only difference between the code snippet in
Figure~\ref{src:lru} is the addition of lines 2-7.  We trigger the policy
switch at 100K keys to absorb the flash crowd at the beginning of the run. Once
triggered, keys are evicted to bring the size of the cache down to the
threshold.  In the bar chart, the cache sizes for the \(n\) key limit policy
...
patterns in Figure~\ref{fig:keyspace-zoomed}. At each time step, we find the
lowest ID and compare against the local minimum, which is the smallest ID we
have seen thus far (Line 7). We also maintain the top and bottom of each fan
(Line 13) so we can tell the ``how much" policy the number of keys to evict
(Line 19).  If we move left to right, the local minimum never changes because
the local minimum will start small. If we move from right to left, the local
minimum changes at each access regime.  For points \(z\), \(y\), and \(x\), if
the local minimum is the same we are in a regime.  Processing \(y\), we set the
...
whole fan, not just the bottom edge of each fan. We exchange runtime, in the
form of an O(n) algorithm, for simplicity. This approach avoids adding new
thresholds for new fan detection, such as space between key accesses, space
