...
appear in the trace.  The trace is anonymized so all file names are replaced
with a unique identifier and we do not know what clients are doing.  Although
other access pattern detection algorithms are possible, we use the one designed
for cache management in Section~\S\ref{sec:regime-detection} without
modification and it successfully detects the largest group of key accesses that
...
in that group of key accesses on the same server to reduce RPCs.
...
reservation to move
  ...
  \item Scheduling: how much of a resource to allocate; more than just LRU
(RAD)

  \item Batching: how many operations to group together and when to send large
batches of updates

  \item Prefetching: how much to prefetch and how to select data to reterive
(forward/backward/stride detection algorithms for prefetching,
