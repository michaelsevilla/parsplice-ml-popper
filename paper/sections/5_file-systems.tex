...
we designed and evaluated above for cache management in ParSplice. 
...
for cache management. This generalization may reduce the work that needs to be
done for cache management as ideas may have already been explored and could
work ``out-of-the-box". 
...
many distributed file systems spread the hierarchical namespace across a
dedicated metadata cluster and caching policies could be used to determine what
metadata to move and when to move it. To demonstrate this idea, consider
Figure~\ref{fig:trace-atime}, where the dots are the file system metadata reads
in a 1 hour window of a 3-day Lustre namespace trace and the vertical lines are
the groups of accesses detected by our policy from
Section~\S\ref{sec:regime-detection}.  The \(x\) axis is time and the \(y\)
axis is the file ID, where the file IDs are listed in the order that they
appear in the trace.  

The trace is anonymized so all file names are replaced with a unique identifier
and we do not know what clients are doing.  Although other access pattern
detection algorithms are possible, we use the one designed for cache management
in Section~\S\ref{sec:regime-detection} with slight modifications based on our
knowledge of file systems~\footnote{We filtered requests key ids less than
2000, as these are most likely path traversal requests to higher parts of the
namespace, {\it i.e.} root directories.} and it successfully detects the
largest group of key accesses that starts at time 1000 seconds and ends at time
2200 seconds. File systems that load balance file system metadata across a
cluster would want to keep metadata in that group of key accesses on the same
server to reduce RPCs.
