...
in~\cite{sevilla:sc15-mantle}. That policy migrates load if the metadata load
is higher than the average load in the cluster {\it and} the current load has
been overloaded for more than two iterations. The two policies have the
following in common:
...
clients and servers maintain a consistent cache, the client can do operation
...
keep local to a server. For example, applying the regime detection algorithm from
...
system could cache none of the metadatas from the \texttt{untar} phase and run
...
automatically size and manage cache tiers
\footnote{http://docs.ceph.com/docs/master/rados/operations/cache-tiering/},
...
something more clever than LRU would improve cache utilization and without
